...
class SpikeAmplitudePlotInner extends TimeWidget {
    ...
    paintMainPanel = (painter) => {
        if (!this.state.unit_ids) return;
        let trange = this.timeRange();
        for (let unit_id of this.state.unit_ids) {
            let times0 = this.state.spike_trains[unit_id];
            let amps0 = this.state.spike_amplitudes[unit_id];
    ...
    _updateAmpRange() {
        ...
        let min = 0, max = 0;
        for (let unit_id of this.state.unit_ids) {
            for (let a of this.state.spike_amplitudes[unit_id]) {
                if (a < min) min = a;
                if (a > max) max = a;
            }
        }
